...
export default function UploadScreen() {
    ...
    async function addItem(kind: Item['kind'], uri: string, filename?: string){
        ...
        const check = await sendToAPI(newItem);
        if(!check) return;
    ...
    const sendToAPI = async (note: Item): Promise<boolean> => {
        if(!connected){
        ...
        Alert.alert("Success", "Note was sent to API."); return true;
        } catch (error) {
        Alert.alert("Error", "Note wasn't sent to API."); return false;
        }
    };
