...
let onEdit: ((item: Item) => void) | null=null;

export function setOnNoteEdit(handler: (item: Item) => void){
    onEdit = handler;
}

export function emitEditNote(edit: Item){
    notes = notes.map((note) => (note.id === edit.id ? edit : note));
    onEdit?.(edit);
}